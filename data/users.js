...
// async function ValidateUser(username,pw)
// {
//     if(!username || !pw) throw "for Validate the user you should provide the username and password"
//     const _user= await users()
//     const userFoundbyUserName = await _user.findOne({username : username})
//     if(!userFoundbyUserName)
//         return false
//     else
//     {
//         let isSame = await bcrypt.compare(pw, userFoundbyUserName.password);
//         if(isSame)
//             return true;
//         else 
//             return false;
...
async function create(firstName, lastName, emailID, uName, pwd,score,avatar,superpowers){
...
{
    if(!userData) throw "YOu should Provide UserData"
  /*  let updateUser = {
    ...
    let updatedUser = await _users.updateOne(
        {_id: ObjectId(id)}, 
    ...
    if(updatedUser === undefined || updatedUser === null) throw "Update User Failed"
    return updatedUser
}
...
async function getUserByName(name) {
    ...
    for (i=0; i<user.length; i++){
        if((user[i].firstName.toLowerCase()).includes(name.toLowerCase())|| (user[i].lastName.toLowerCase()).includes(name.toLowerCase()) ){
          p.push(user[i])
          counter++;
          if(counter>19){
            return p
...
//     try{
//     var manoj = await create("manoj", "salvi", "mrunal@gmail.com", "manu", "mimadhur!@#$1A")
//         console.log(manoj)

//         var andy = await create("john", "pimple", "heavymetalani@gmail.com", "heavymetalani", "hopesh!@#$1A")
//     console.log(andy)

//     var atul = await create("atul", "kundan", "aakash@gmail.com", "pussydestroyer", "jaishivji!@#$1A")
//     console.log(atul)

//     var pete = await create("petesh", "sherman", "protogdyt@gmail.com", "prothepsmesh", "mustdie!@#$1A")
//     console.log(pete)
